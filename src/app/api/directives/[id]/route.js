/**
 * Directives [id] — PUT update/archive a directive.
 * Proxies to Mission Control PUT /api/directives/:id
 */
...
export async function PUT(request, { params }) {
  try {
    ...
    const res = await fetch(`${MC_URL}/api/directives/${safeId}`, {
      method: 'PUT',
      headers: mcHeaders({ 'Content-Type': 'application/json' }),
      body: JSON.stringify({ ...body, editor: session.email || session.name }),
      cache: 'no-store',
    ...
    if (!res.ok) {
      return NextResponse.json({ error: 'Failed to update directive' }, { status: res.status >= 500 ? 502 : res.status });
    }
    const data = await res.json();
    return NextResponse.json(data);
  } catch (err) {
    console.error('[directives/:id] PUT error:', err);
    return NextResponse.json({ error: 'Failed to update directive', detail: String(err) }, { status: 500 });
  }
}
